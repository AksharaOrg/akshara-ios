...
enum SinhalaEngine {
    ...
    static func transliterate(_ source: String, mode: Mode) -> String {
        ...
        while index < source.endIndex {
            ...
            if character == "M" || (mode == .smartPhonetic && character == "x") {
            ...
            if let (key, consonant) = match(consonants) {
                ...
                if let (vowelKey, _, vowelSign) = matchVowel() {
                    ...
                    // Preserve the shaping behavior used by the existing macOS implementation.
                    output += (nextKey == "y" || (nextKey == "r" && !["m", "n", "l"].contains(key))) ? "්‍" : "්"
                } else {
                    output += "්"
                }
                continue
